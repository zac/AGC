...
After idle boot, `bootAndEnterP63` loads NASA Luminary 99 landing-guidance pad-loads (TLAND through TAUVERT), launch-tape TEPHEM/AZO/504LM and NASA RLS at ECADR 02022, a PDI-relative GET clock (`TLAND − GUIDDURN − ZOOMTIME`), RN/VN as moon-fixed offsets from that site, identity REFSMMAT, and MODE CONTROL AUTO / auto-throttle / LR POS1. The frame loop answers P63’s remaining crew flashes: ENTER on V50N25 skips R51 fine-align, and PROCEED is held 150 ms on V50N18 and V99. RN is still over the site (not ~260 nmi uprange) and still moon-fixed while IGNALG `RP-TO-R`s RLS into inertial. Body angular rates and the DPS engine-to-CG moment arm are still unmodeled. Do not invent those just to animate a landing.
